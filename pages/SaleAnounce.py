...
# 3. Пошук по EMAIL (ЗМІНЕНО)
search_query = st.sidebar.text_input("Пошук за email власника:")
...
# Логіка фільтрації змінена на owner_email
if search_query:
    filtered_data = filtered_data[filtered_data['owner_email'].str.contains(search_query, case=False, na=False)]
...
selected_ann_id_for_details = st.selectbox("Оберіть ID оголошення для перегляду характеристик:",
                                           options=filtered_data['announcement_id'])

if selected_ann_id_for_details:
    car_id_result = run_query("SELECT car_id FROM public.\"Sale_Announcements\" WHERE announcement_id = %s",
                              (selected_ann_id_for_details,), fetch="one")
    if car_id_result:
...
if operation == "Редагувати":
    ...
    ann_to_update = st.selectbox("Оберіть ID оголошення для редагування:", options=filtered_data['announcement_id'],
                                 key="upd_ann")
    if ann_to_update:
        ...
        with st.form("update_ann_form"):
            ...
            if st.form_submit_button("Оновити оголошення"):
                run_query(
                    'UPDATE public."Sale_Announcements" SET price = %s, description = %s WHERE announcement_id = %s;',
                    (new_price, new_description, ann_to_update))
    ...
    ann_to_archive = st.selectbox("Оберіть ID оголошення для архівування:", options=filtered_data['announcement_id'],
                                  key="archive_ann")

    if st.button("Архівувати оголошення"):
        # Увага: перевір, чи підтримує твій ENUM статус 'archived'.
        # Якщо ні, зміни на 'inactive'.
        run_query('UPDATE public."Sale_Announcements" SET status = %s WHERE announcement_id = %s;',
                  ('inactive', ann_to_archive))
        st.success(f"Оголошення {ann_to_archive} перенесено в архів (status='inactive')!")
        st.cache_data.clear()
        time.sleep(1)
        st.rerun()